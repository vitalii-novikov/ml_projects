...
The accuracy of predictions is better for 1 hidden layer neural network, but the cross entropy loss for these networks were larger than network with 4 and 3 neurons on hidden levels. But (4,3) nn shows average accuracy within networks with 2 hidden layers.
...
set.seed(2)
text(x = vt$accuracy_avg, y = vt$cross_entropy_avg, labels = vt$node, pos = 3, cex = 0.5)
```

Another possible neural network, that could be good choice for using is nn with 1 hidden layer and 4 neurons on this layer, because it is the closest to right bottom corner, which means that the model shows good balance of accuracy and loss function.

But we would like to choose the model with the lowest cross entropy loss, so the (4,3) is a winning model.
...
So, the accuracy is 100%, while the loss is very small comparing to models that were on previous steps.

```{r}
plot(nn, col.hidden = "orange", fontsize = 10, information=F)
```

The black lines show the connections between each layer and the weights on each connection while the blue lines show the bias term added in each step. Black neurons on the left side is input, while Black neurons on the right side is output. Orange neurons are part of hidden layers.